...
def score_set(set, testid, refids, n=4):
    ...
    for seg in set.segs():
        try:
            ...
            sys.stderr.write("Warning: missing test sentence\n")
            continue
        try:
            refs = [seg.versions[refid].words for refid in refids]
        except KeyError:
            sys.stderr.write("Warning: missing reference sentence, %s\n" % seg.id)
